<?php
/**
 *
 * Localization file uk/position
 */
return array(
    'Title' => 'Назва посади',
    'Create new position' => 'Створити нову посаду',
    'Updating position' => 'Оновлення посади',
    'Positions list' => 'Список посад',
    'View position' => 'Переглянути посаду',
    'Remove position' => 'Вилучити посаду',
    'Manage position' => 'Керувати посадою',
    'Max Load Hour 1' => 'Максимальна кількість навантажених годин 1',
    'Max Load Hour 2' => 'Максимальна кількість навантажених годин 2',
    'Select position' => 'Оберіть посаду',
    'Position name' => 'Назва посади, професії',
    'Position code' => 'Код посади, професії',
    'Category, salary'=>'Розряд (оклад)',
);